...
vertexOutput2 GlassVS(appdata IN,
...
float4 GlassPS(vertexOutput2 IN,
    ...
    ) : COLOR 
{
  ...
  float3 specContrib = res.specContrib;
  float3 result = ((SurfColor)*Kd*res.diffContrib)+reflColor+AmbiColor+(Ks*SpecColor*specContrib);
  return float4(result.xyz,Transparence);
}
...
technique CarPaint
{
	...
	{		
		...
		VertexProgram = compile vp40 GlassVS(wvp, worldIT, world, viewIT);
		FragmentProgram = compile fp40 GlassPS(
			EnvironmentMap,
