...
Help()
{
    ...
    echo ' '
    echo '    '$SHELLNAME' -h'
    echo '    '$SHELLNAME' [-env file -direnv dir] [-installdir dir] V4ModelName.model'
    echo ' '
    ...
    then
		...
		# Directory where the CATIAV5 environment file is stored
			...
			then
				...
				# Argument is the V4 model name
				else
						# option already detected
						if [ $MODELNAMEFOUND -eq 1 ]
						then
								# unexpected argument
...
# Launch migration
$WSOSCODEBIN/CNEXT $V5ENV $V5ENVFILE $V5DIRENV $V5DIRENVPATH -batch -e CATAecV4ToV5ModelParts $MODELNAME 2>&1
