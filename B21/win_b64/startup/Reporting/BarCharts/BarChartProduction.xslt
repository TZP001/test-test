...
<!--Style Sheet to Convert XML file to Production Bar Chart-->
<!--
	...
	displays the Production of the Resources selected.
-->
<xsl:stylesheet version="1.0" xmlns="http://www.w3.org/2000/svg"
  ...
  <xsl:param name="RunNumber"/>
	...
	<xsl:variable name="verticalRangeMax">
	    <xsl:for-each select="/SystemStatistics/Run[@RunNumber=$RunNumber]/Resources/Sinks/Sink/ProductStatistics/AllProducts/NumDestroyed">
	...
	<xsl:variable name="BarStartDistance" select="$horizontalScale div 6"/>
	...
	<!-- Getting the Production Color from the Color Param input variable-->
	<!-- NOTE : We cant modify a global variable once created. Hence below
	            we are using new variables in each line -->
	<xsl:variable name="ProductionRed" select="substring-before($ColorParam,';')"/>
	<xsl:variable name="tempProductionGreen" select="substring-after($ColorParam,';')"/>
	<xsl:variable name="ProductionGreen" select="substring-before($tempProductionGreen,';')"/>
	<xsl:variable name="tempProductionBlue" select="substring-after($tempProductionGreen,';')"/>
	<xsl:variable name="ProductionBlue" select="substring-before($tempProductionBlue,';')"/>
         ...
         stroke-linejoin="round" stroke-miterlimit="10">
	      <g id="GridAndLegend" style="stroke:none;">
		      <g stroke="black">
		        ...
		        <text stroke="black" font-size="20" transform="translate({$chartStartX + 10},-10)">Bar Chart for Resource Production</text>
		        ...
		        <xsl:comment>
			        Production Red = <xsl:value-of select="$ProductionRed"/>
		        </xsl:comment>
		        <xsl:comment>
			        Production Green = <xsl:value-of select="$ProductionGreen"/>
		        </xsl:comment>
		        <xsl:comment>
			        Production Blue = <xsl:value-of select="$ProductionBlue"/>
		        </xsl:comment>
		        <xsl:comment>
		      ...
		      <text transform="translate(0 ,{$chartHeight + 15})  rotate(-90)">No. of Parts Produced</text>
		      ...
		      <g style="fill:rgb({$ProductionRed}, {$ProductionGreen}, {$ProductionBlue}); stroke-width: 0.9180;stroke-linecap:butt; stroke-linejoin:miter; stroke-dasharray:none">
		          <path d="M {$chartWidth + $chartStartX + 35},{$chartHeight + 15} v -40 h 10 v 40 h -10"/>
		          <text transform="translate( {$chartWidth + $chartStartX + 35 + 20}, {$chartHeight + 15 - 20})">Production</text>
		      </g>
	      </g>
	    </g>
    </svg>
	...
	<xsl:template name="Tokenize">
		...
		<xsl:apply-templates select="Run[@RunNumber=$RunNumber]//Sink[@Name=$ResourceToken]">
	...
	<xsl:template match="Sink">
		...
		<xsl:if test="(./ProductStatistics/AllProducts/NumDestroyed &gt; 0) or (./ProductStatistics/AllProducts/NumDestroyed = 0)">
			<xsl:call-template name="DrawProductionBarChart">
				<xsl:with-param name="position" select="$position"/>
				<xsl:with-param name="totalParts" select="./ProductStatistics/AllProducts/NumDestroyed"/>
			</xsl:call-template>
		</xsl:if>
	...
	<xsl:template name="DrawProductionBarChart">
		<xsl:param name="position"/>
		<xsl:param name="totalParts"/>
		
		<xsl:comment>
			Entering DrawProductionBarChart TEMPLATE!!!
		</xsl:comment>
		
		<xsl:comment>
			Position = <xsl:value-of select="$position"/>
			Total Parts = <xsl:value-of select="$totalParts"/>
		</xsl:comment>

		<xsl:variable name="BarStartDistance" select="$BarStartDistance + ($horizontalScale * ($position - 1))"/>
		<xsl:variable name="barHeight" select="$totalParts * $verticalScale"/>
		<xsl:variable name="tempX" select="$chartStartX + $BarStartDistance + $barWidth"/>
		<xsl:variable name="tempY" select="$chartHeight + 15 - $barHeight"/>
		<xsl:variable name="textX" select="$chartStartX + $BarStartDistance + 15"/>
		<xsl:variable name="textY" select="$chartHeight + 15 - $barHeight - 15 - 5"/>-->

		<xsl:comment>====== Drawing Production Bar Chart ======</xsl:comment>
		<path d="M {$tempX},{$tempY} l 15,-15 v {$barHeight} l -15,15 z" stroke="black" 
          fill="rgb({$ProductionRed - 80}, {$ProductionGreen - 80}, {$ProductionBlue - 80})"/> <!-- Right Side of Bar -->
		<path d="M {$tempX},{$tempY} h -{$barWidth} l 15,-15 h {$barWidth} z" stroke="black" 
          fill="rgb({$ProductionRed - 40}, {$ProductionGreen - 40}, {$ProductionBlue - 40})"/> <!-- Top Side of Bar -->
		<path d="M {$tempX},{$tempY} h -{$barWidth} v {$barHeight} h {$barWidth} z" stroke="black" 
          fill="rgb({$ProductionRed}, {$ProductionGreen}, {$ProductionBlue})"/> <!-- Front Side of Bar -->
		<text transform="translate( {$textX} , {$textY})">
			<xsl:value-of select="$totalParts"/>
		</text>
		...
		<xsl:if test="$AnnotationParam = 'TRUE'">
			...
			<xsl:variable name="AnnotX" select="$chartStartX + $BarStartDistance + ($barWidth div 2) - 5"/>
