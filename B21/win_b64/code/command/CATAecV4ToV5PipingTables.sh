...
Help()
{
    ...
    echo ' '
    echo '    '$SHELLNAME' -h'
    echo '    '$SHELLNAME' [-env file -direnv dir] [-installdir dir] V4TableList'
    echo ' '
    ...
    then
		...
		# Directory where the CATIAV5 environment file is stored
			...
			then
				...
				# Argument is the V4 Model
				else
						# option already detected
						if [ $TABLELISTFOUND -eq 1 ]
						then
								# unexpected argument
...
if [ $TABLELISTFOUND -ne 1 ]
then
    echo 'ERROR!! Table List file was not specified!'
    Help 1
fi
...
# Launch migration
$WSOSCODEBIN/CNEXT $V5ENV $V5ENVFILE $V5DIRENV $V5DIRENVPATH -batch -e CATAecV4ToV5PipingTables $TABLELIST ALL ASTL 2>&1
